# PQ4 Blackbox Testing Template

This repository contains the interface and a sample "dummy" implementation of GatorBST, a simplified binary search tree used to store student ID's and names.

You will be performing **black box testing** - this means that you will not be able to see the code that you are writing tests for! You must instead write tests to validate functionality based on the provided interface in [GatorBST.h](./src/GatorBST.h). You will only submit your test.cpp file to GradeScope for grading.

You may work on your test suite in [test/test.cpp](./test/test.cpp). You can run the DummyTests executable to ensure that your tests compile.

We will run your tests against a correct implementation. The percentage of your tests that pass against this implementation will be your score for Part A.

We will also run your tests against a number of bugged implementations (often called "mutants") to see if they can successfully catch the bugs. You will get credit for catching a bug if at least one of your tests fails when run against the buggy implementation **AND** that failing test passed for the correct implementation.

You will also write a short reflection for 5% of the assignment grade. (TODO: Add rubric details)

While you are welcome to expand upon dummy.cpp with some implementation code to see how your tests behave, we will grade your tests' effectiveness against *our* implementations. 

Carefully writing tests for this assignment will give you a head start for the GatorAVL project, which requires a similar set of core BST functions (but also will involve node *balancing*).

This template is based upon the existing progamming quiz 3 template and project templates and has largely the same workflow, except you will only be running one executable "DummyTests". Please refer to the following instructions copied from the other repositories for setup help.
